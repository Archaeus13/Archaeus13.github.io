...
\subsubsection{数项级数}
\begin{framed}
\begin{exercise}\label{an div sn}
    设$a_n$为正数列，其部分和数列为$S_n$，且$S_n$极限为$A$\ (可能为有限数或正无穷)，对$\alpha>0$，判断
    $$\suminf\frac{a_n}{S_n^\alpha}$$
    的敛散性。
\end{exercise}
\sol{
    若$A$为有限数，也即$\suminf a_n$收敛，利用\textbf{比较判别法}，由
    $$\lim_{n\to\infty}\frac{a_n}{\frac{a_n}{S_n^\alpha}}=\lim_{n\to\infty}S_n^\alpha=A^\alpha$$
    可知$\suminf\frac{a_n}{S_n^\alpha}$也\textbf{收敛}。

    若$A$为正无穷，也即$\suminf a_n$发散，通过尝试特定的$a_n$可发现收敛与发散基本以$\alpha=1$为临界情况，由此我们将证明分为三个部分：
    \begin{itemize}
        \item \textbf{临界情况$\alpha=1$}
        
        对具体$a_n$尝试后可猜测此时\textbf{发散}。由于没有简单的方法说明敛散性，我们尝试直接使用\textbf{柯西收敛原理}。先估算求和(由$a_n$为正数列可去除绝对值)
        $$\sum_{k=n+1}^m\frac{a_k}{S_k}$$
        为了进行放缩，我们将$S_k$都放大到最后一项$S_m$，此时即有
        $$\sum_{k=n+1}^m\frac{a_k}{S_k}\ge\sum_{k=n+1}^m\frac{a_k}{S_m}=\frac{S_m-S_n}{S_m}=1-\frac{S_n}{S_m}$$
        固定$n$可发现
        $$\lim_{m\to\infty}1-\frac{S_n}{S_m}=1$$
        由此利用极限\textbf{保序性}，对任何正整数$n$，存在$m>n$使得
        $$1-\frac{S_n}{S_m}>\frac{1}{2}$$
        此时有
        $$\sum_{k=n+1}^m\frac{a_k}{S_k}>\frac{1}{2}$$
        取定$\varepsilon=\frac{1}{2}$，对任何正整数$N$，取$n=N+1$，并取$m$使得上式成立，即得到这符合\textbf{柯西收敛原理的否定}，从而证明了原级数发散。

        \item \textbf{$\alpha<1$情况}
        
        由于$\lim_{n\to\infty}S_n=+\infty$，利用极限\textbf{保序性}可知存在正整数$N$使得$n>N$时$S_n>1$，从而此时有
        $$\frac{a_n}{S_n^\alpha}\ge\frac{a_n}{S_n}$$
        利用前一部分结论与\textbf{比较判别法}可知此时原级数发散。

        \item \textbf{$\alpha>1$情况}

        这里需要一个具有一定技巧性的\textbf{积分放缩}。对所有$n\ge2$，$a_n$都可以写为$S_n-S_{n-1}$，而将其放大为积分可知
        $$\frac{a_n}{S_n^\alpha}=\frac{S_n-S_{n-1}}{S_n^\alpha}\le\int_{S_{n-1}}^{S_n}\frac{\dr x}{x^\alpha}$$
        由此求和即有
        $$\sum_{n=2}^m\frac{a_n}{S_n^\alpha}\le\int_{S_1}^{S_m}\frac{\dr x}{x^\alpha}=(\alpha-1)(S_1^{1-\alpha}-S_m^{1-\alpha})<(\alpha-1)S_1^{1-\alpha}$$
        利用\textbf{单调有界定理}，从正数列$\frac{a_n}{S_n^\alpha}$的部分和有上界即可得到收敛性。
    \end{itemize} 
}
\end{framed}
...
\begin{framed}
\begin{exercise}
    设正项级数$\suminf a_n$发散，证明存在发散的正项级数$\suminf b_n$使得
...
\sol{
    ...
    \begin{itemize}
        ...
        当$c>0$时，利用极限\textbf{保序性}可知存在$N$使得$n>N$时$\frac{u_n}{v_n}>0$，又由$v_n>0$可知$n>N$时$u_n>0$，从而$u_n$去除前$N$项(这不影响敛散性)后即成为正项级数，利用正项级数比较判别法可知与$v_n$敛散性相同。
...
\note 利用柯西判别法，幂级数的收敛半径只与系数的\textbf{指数部分}有关，因此放缩只需保证\textbf{指数部分的阶}不变即可。
...
\sol{
    ...
    \begin{itemize}
        ...
        $$\frac{8n+5}{(4n+1)(4n+3)(2n+2)}\le\frac{13n}{(4n)^2\cdot2n}\le\frac{13}{32}\cdot\frac{1}{n^2}$$
        由\textbf{$p$级数}敛散性结论与数项级数基本性质可知$\suminf\frac{13}{32n^2}$收敛，于是根据\textbf{比较判别法}得左侧也收敛，这就说明了$S_1(1)$、$S_2(1)$收敛。
...
\sol{
    ...
    \begin{itemize}
        ...
        由于傅里叶级数本身的计算复杂性，我们往往希望利用\textbf{多项式}进行配凑。由此，我们先进行分部积分得到一些结果，对\textbf{正整数}$n$有(计算过程类似\exref{fourier n4}与\exref{fourier n6})：
        ...
        可利用$x^2$消去第一项得到
        $$\int_0^\pi(2x^3-3\pi x^2)\cos(nx)\dr x=\frac{12(1-(-1)^n)}{n^4}$$
        ...
        $$S=\suminf\frac{1}{n^8}$$
        我们先根据定义写出
        $$\sum_{n=1}^m\frac{1}{(2n-1)^8}=\sum_{n=1}^{2m}\frac{1}{n^8}-\sum_{n=1}^m\frac{1}{(2n)^8}=\sum_{n=1}^{2m}\frac{1}{n^8}-\sum_{n=1}^m\frac{1}{256n^8}$$
        在上式中令$m\to\infty$，则第一个级数趋于$S$\ (它的部分和是$\suminf\frac{1}{n^8}$部分和的\textbf{子列})，第二个级数趋于$\frac{S}{256}$，由此最终得到
...
\sol{
    ...
    \begin{itemize}
        ...
        $$\lim_{x\to1^+}\frac{\ln x\ln(1+x)}{\sqrt{x^p-1}}=\lim_{t\to0^+}\frac{\ln(1+t)\ln(2+t)}{\sqrt{(1+t)^p-1}}=\lim_{t\to0^+}\frac{t\ln 2}{\sqrt{pt}}=0$$
...
\sol{
    ...
    \begin{itemize}
        ...
        $$\lim_{x\to0^+}\frac{x^n(1-\er^{-x})}{(1+x)^m}=\lim_{x\to0^+}x^{n+1}\cdot\frac{1}{(1+x)^m}=\begin{cases}0&n\in(-1,0)\\1&n=-1\end{cases}$$
...
\sol{
    ...
    \begin{itemize}
        ...
        由于$I(a)$为偶函数，最终得到
        $$\forall a\in\rb,\quad I(a)=\pi\ln\frac{|a|+1}{2}$$
    \end{itemize}
}
\end{framed}
...
\note 本题也可以考虑化成\textbf{多重积分}，即利用$\frac{\sin(ax)-\sin(bx)}{x}=\int_a^b\cos(tx)\dr t$再交换积分顺序。
...
\note 对于$\er^{-x^n}$相关的问题，往往可以换元$t=x^n$化为$\Gamma$函数或类似形式处理。本题若是想不到这样的算法，\textbf{难以直接估算出结果}，因为需要拆分为$[0,1]$与$[1,+\infty)$再对每段做分段估算。
...
\sol{
    ...
    \begin{itemize}
        ...
        另一方面，由定积分性质可得
        $$\sum_{n=1}^M\int_{a_{n-1}}^{a_n}f(x)\dr x=\int_{a_0}^{a_M}f(x)\dr x$$
        ...
        于是
        $$F(x)-A\le F(a_{N_0})-A<\varepsilon,\quad F(x)-A\ge F(a_{N+1})-A>-\varepsilon$$
        这就得到了
        $$|F(x)-A|<\varepsilon$$
...
\begin{exercise}
    设对正整数$n$有
    $$f_1(x)=\int_0^x\sqrt{1+t^4}\dr t,\quad f_{n+1}(x)=\int_0^xf_n(t)\dr t$$
    证明
    $$\suminf f_n(x)$$
...
\sol{
    ...
    \begin{itemize}
        ...
        $$\suminf a_n\ge r_1\cdot\frac{\varepsilon}{r_1}+r_2\bigg(\frac{\varepsilon}{r_2}-\frac{\varepsilon}{r_1}\bigg)$$
        以此类推，每次增加$r_m(\frac{\varepsilon}{r_m}-\frac{\varepsilon}{r_{m-1}})$，我们可以得到对正整数$m$，求和
        $$\varepsilon+\sum_{n=2}^mr_n\bigg(\frac{\varepsilon}{r_n}-\frac{\varepsilon}{r_{n-1}}\bigg)$$
        有上界。

        设$b_1=\frac{\varepsilon}{r_1}$，且对大于等于2的正整数$m$有$b_m=\frac{\varepsilon}{r_m}-\frac{\varepsilon}{r_{m-1}}$，可发现$b_m$的前$n$项和
        $$T_n=\frac{\varepsilon}{r_n}$$
        由此上述求和可改写为
        $$\sum_{n=1}^m\varepsilon\frac{b_n}{T_n}$$
        由$T_n$极限为$+\infty$可知$\suminf b_n$是\textbf{发散的正项级数}，利用\exref{an div sn}可知求和发散，矛盾。
    \end{itemize}
